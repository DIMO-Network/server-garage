...
func TestWithCondensedSchema(t *testing.T) {
	...
	mcpHandler, err := New(mockGQLExecutor(), "Test Server", "0.1.0", "test", WithCondensedSchema(condensedSDL))
...
func TestNewWithoutCondensedSchemaErrors(t *testing.T) {
	_, err := New(mockGQLExecutor(), "Test Server", "0.1.0", "test")
	require.Error(t, err)
	assert.Contains(t, err.Error(), "WithCondensedSchema is required")
}
